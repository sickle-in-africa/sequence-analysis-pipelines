...
while true; do
    case "$1" in
      ...
      -d|--dbsnp) dbsnp="$(readlink -f $2)";
         ...
         shift 2
         ;;
      -h|--help) shift; usage; continue; 1>&2; exit 1 ;;
...
if [[ $? != 0 ]]; then
	 ...
	 *) if [[ ( "$1" =~ --h ) || ( "$1" =~ --help ) ]]; then usage; else echo -e "\nInvalid command! Please enter at least one command. Run geneMapNGS.sh [-h|--help] for usage\n"; fi; 1>&2; exit 1 ;;
#         *) if [[ -z "$1"  ]]; then echo " "; shift; break; else shift; break; fi ;;
      esac
      continue
    done
fi
